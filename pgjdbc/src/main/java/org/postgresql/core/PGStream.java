...
public class PGStream implements Closeable, Flushable {
	...
	/**
	 * Constructor: Connect to the PostgreSQL back end and return a stream
	 * connection.
	 *
	 * @param netClient
	 *            net client to use when creating sockets
	 * @param hostSpec
	...
	/**
	 * Check for pending backend messages without blocking. Might return false when
	 * there actually are messages waiting, depending on the characteristics of the
	 * underlying socket. This is used to detect asynchronous notifies from the
	 ...
	 *            the integer to be sent
	 * @throws IOException
	 *             if an I/O error occurs or {@code val} cannot be encoded in 2
	 *             bytes
	 */
	...
	/**
	 * Send a fixed-size array of bytes to the backend. If {@code length < siz}, pad
	 * with zeros. If {@code length > siz}, truncate the array.
	 *
	 * @param buf
	 *            the array of bytes to be sent
	 * @param off
	 *            offset in the array to start sending from
	 * @param siz
	 *            the number of bytes to be sent
	 * @throws IOException
	 *             if an I/O error occurs
	...
	/**
	 * Receives a single character from the backend, without advancing the current
	 * protocol stream position.
	 *
	 * @return the character received
	 * @throws IOException
	 *             if an I/O Error occurs
	 */
	public CompletableFuture<Integer> peekChar() {
	 ...
	 *             if an I/O Error occurs
	 */
	public CompletableFuture<Integer> receiveChar() {
	 ...
	 *             if an I/O error occurs
	 */
	public CompletableFuture<Integer> receiveInteger4() {
		byte[] buf = new byte[4];
		await(this.stream.read(buf));
		return CompletableFuture
				.completedFuture((buf[0] & 0xFF) << 24 | (buf[1] & 0xFF) << 16 | (buf[2] & 0xFF) << 8 | buf[3] & 0xFF);
	}
	 ...
	 *             if an I/O error occurs
	 */
	public CompletableFuture<Integer> receiveInteger2() {
		byte[] buf = new byte[2];
		await(this.stream.read(buf));
	 ...
	 *
	 * @param len
	 *            the length of the string to receive, in bytes.
	 * @return the decoded string
	 * @throws IOException
	 *             if something wrong happens
	 */
	public CompletableFuture<String> receiveString(int len) throws IOException {
	...
	/**
	 * Receives a fixed-size string from the backend, and tries to avoid "UTF-8
	 ...
	 *             if something wrong happens
	 */
	public CompletableFuture<EncodingPredictor.DecodeResult> receiveErrorString(int len) throws IOException {
		byte[] buffer = new byte[len];
		try {
			await(this.stream.read(buffer));
		} catch (Throwable err) {
		...
		try {
			...
			return CompletableFuture.completedFuture(new EncodingPredictor.DecodeResult(value, null));
		} catch (IOException e) {
			res = EncodingPredictor.decode(buffer, 0, len);
			if (res == null) {
				Encoding enc = Encoding.defaultEncoding();
				String value = enc.decode(buffer, 0, len);
				return CompletableFuture.completedFuture(new EncodingPredictor.DecodeResult(value, enc.name()));
			}
	...
	/**
	 * Receives a null-terminated string from the backend. If we don't see a null,
	 * then we assume something has gone wrong.
	 *
	 * @return string from back end
	 * @throws IOException
	 *             if an I/O error occurs, or end of file
	 */
	public CompletableFuture<String> receiveString() throws IOException {
		try {
			byte[] data = await(this.stream.readUntil((byte) '\0'));
			return CompletableFuture.completedFuture(encoding.decode(data, 0, data.length - 1));
		} catch (Throwable err) {
			throw new IOException(err.getMessage(), err);
		}
	}
	 ...
	 * This variant reads the V3 protocol's tuple representation.
	 *
	 * @return tuple from the back end
	 * @throws IOException
	 *             if a data I/O error occurs
	 */
	public CompletableFuture<byte[][]> receiveTupleV3() throws IOException, OutOfMemoryError {
		// TODO: use l_msgSize
		int l_msgSize = await(receiveInteger4());
		int l_nf = await(receiveInteger2());
		byte[][] answer = new byte[l_nf][];

		OutOfMemoryError oom = null;
		for (int i = 0; i < l_nf; ++i) {
			int l_size = await(receiveInteger4());
			if (l_size != -1) {
				try {
	 ...
	 *             if a data I/O error occurs
	 */
	public CompletableFuture<byte[]> receive(int siz) throws IOException {
		byte[] answer = new byte[siz];
		await(receive(answer, 0, siz));
		return CompletableFuture.completedFuture(answer);
	}
	 ...
	 *             if a data I/O error occurs
	 */
	public CompletableFuture<Void> receive(byte[] buf, int off, int siz) throws IOException {
		try {
			await(this.stream.read(buf, off, siz));
		} catch (Throwable err) {
			throw new IOException(err.getMessage(), err);
		}
	 ...
	 */
	public CompletableFuture<Void> receiveEOF() throws IOException {
		CompletableFuture<Void> result = new CompletableFuture<>();
		this.stream.read().whenComplete((value, error) -> {
			if (error != null) {
			...
			if (value < 0) {
				...
				result.completeExceptionally(new PSQLException(GT.tr("Expected an EOF from server, got: {0}", value),
