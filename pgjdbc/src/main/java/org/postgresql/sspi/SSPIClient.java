...
/**
 * Use Waffle-JNI to support SSPI authentication when PgJDBC is running on a
 * Windows client and talking to a Windows server.
 *
...
public class SSPIClient implements ISSPIClient {
	 ...
	 *
	 * It is safe to instantiate SSPIClient even if Waffle and JNA are missing or on
	 * non-Windows platforms, however you may not call any methods other than
	 * isSSPISupported().
	 *
	 * @param pgStream
	 *            PostgreSQL connection stream
	 * @param spnServiceClass
	 *            SSPI SPN service class, defaults to POSTGRES if null
	 * @param enableNegotiate
	 *            enable negotiate
	 */
	...
	/**
	 * Test whether we can attempt SSPI authentication. If false, do not attempt to
	 * call any other SSPIClient methods.
	 *
	...
	public boolean isSSPISupported() {
		try {
			/*
			 * SSPI is windows-only. Attempt to use JNA to identify the platform. If Waffle
			 * is missing we won't have JNA and this will throw a NoClassDefFoundError.
			 */
	...
	private String makeSPN() throws PSQLException {
		...
		try {
			return NTDSAPIWrapper.instance.DsMakeSpn(spnServiceClass, hs.getHost(), null, (short) hs.getPort(), null);
		} catch (LastErrorException ex) {
			throw new PSQLException("SSPI setup failed to determine SPN", PSQLState.CONNECTION_UNABLE_TO_CONNECT, ex);
		}
	}

	/**
	 * Respond to an authentication request from the back-end for SSPI
	 * authentication (AUTH_REQ_SSPI).
	 *
	 * @throws SQLException
	 *             on SSPI authentication handshake failure
	 * @throws IOException
	...
	public void startSSPI() throws SQLException, IOException {
		...
		try {
			 ...
			 * (http://msdn.microsoft.com/en-us/library/windows/desktop/aa374712%28v=vs.85%
			 * 29.aspx)
			 *
			...
			throw new PSQLException("SSPI cannot be used, Waffle or its dependencies are missing from the classpath",
					PSQLState.NOT_IMPLEMENTED, ex);
		}
	}

	/**
	 * Continue an existing authentication conversation with the back-end in resonse
	 * to an authentication request of type AUTH_REQ_GSS_CONT.
	 *
	 * @param msgLength
	 *            Length of message to read, excluding length word and message type
	 *            word
	 * @throws SQLException
	 ...
	 *             if something wrong happens
	 */
	public CompletableFuture<Void> continueSSPI(int msgLength) throws SQLException, IOException {
		...
		byte[] receivedToken = await(pgStream.receive(msgLength));
		...
		/*
		 * Now send the response token. If negotiation is complete there may be zero
		 * bytes to send, in which case we shouldn't send a reply as the server is not
		 * expecting one; see fe-auth.c in libpq for details.
		 */
	...
	private void sendSSPIResponse(byte[] outToken) throws IOException {
		/*
		 * The sspiContext now contains a token we can send to the server to start the
		 * handshake. Send a 'password' message containing the required data; the server
		 * knows we're doing SSPI negotiation and will deal with it appropriately.
		 */
		pgStream.sendChar('p');
	...
	/**
	 * Clean up native win32 resources after completion or failure of SSPI
	 * authentication. This SSPIClient instance becomes unusable after disposal.
	 */
	public void dispose() {
		if (sspiContext != null) {
			sspiContext.dispose();
