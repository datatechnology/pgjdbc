...
public class PgStatement implements Statement, BaseStatement {
	...
	private static final AtomicReferenceFieldUpdater<PgStatement, TimerTask> CANCEL_TIMER_UPDATER = AtomicReferenceFieldUpdater
			.newUpdater(PgStatement.class, TimerTask.class, "cancelTimerTask");
	...
	private static final AtomicReferenceFieldUpdater<PgStatement, StatementCancelState> STATE_UPDATER = AtomicReferenceFieldUpdater
			.newUpdater(PgStatement.class, StatementCancelState.class, "statementState");
	...
	public ResultSet createResultSet(Query originalQuery, Field[] fields, List<byte[][]> tuples, ResultCursor cursor)
			throws SQLException {
		PgResultSet newResult = new PgResultSet(originalQuery, this, fields, tuples, cursor, getMaxRows(),
	...
	public boolean execute(String p_sql) throws SQLException {
		try {
			return executeWithFlags(p_sql, 0).get();
		} catch (InterruptedException | ExecutionException e) {
			throw new SQLException(e);
		}
	}
	...
	protected BatchResultHandler createBatchHandler(Query[] queries, ParameterList[] parameterLists) {
		return new BatchResultHandler(this, queries, parameterLists, wantsGeneratedKeysAlways);
	}
	...
	public int[] executeBatch() throws SQLException {
		...
		if ((preDescribe || forceBinaryTransfers) && (flags & QueryExecutor.QUERY_EXECUTE_AS_SIMPLE) == 0) {
			...
			try {
				try {
					connection.getQueryExecutor().execute(queries[0], parameterLists[0], handler2, 0, 0, flags2).get();
				} catch (InterruptedException | ExecutionException e) {
					throw new SQLException(e);
				}
			} catch (SQLException e) {
	...
	public int executeUpdate(String sql, String[] columnNames) throws SQLException {
		...
		try {
			if (!executeCachedSql(sql, 0, columnNames).get()) {
	...
	public boolean execute(String sql, String[] columnNames) throws SQLException {
		...
		wantsGeneratedKeysOnce = true;
		try {
			return executeCachedSql(sql, 0, columnNames).get();
		} catch (InterruptedException | ExecutionException e) {
			throw new SQLException(e);
		}
	}
