...
public class PgResultSetMetaData implements ResultSetMetaData, PGResultSetMetaData {
	...
	/*
	 * Is the column automatically numbered (and thus read-only) I believe that
	 * PostgreSQL does not support this feature.
	 *
	...
	/*
	 * Does a column's case matter? ASSUMPTION: Any field that is not obviously case
	 * insensitive is assumed to be case sensitive
	 *
	 * @param column the first column is 1, the second is 2...
	 *
	 * @return true if so
	 ...
	 * functions into two types: recognised types (which are always useable), and
	 * OTHER types (which may or may not be useable). The OTHER types, for now, I
	 * will assume they are useable. We should really query the catalog to see if
	 * they are useable.
	 *
	 * @param column the first column is 1, the second is 2...
	 *
	...
	/*
	 * Is the column a cash value? 6.1 introduced the cash/money type, which haven't
	 * been incorporated as of 970414, so I just check the type name for both 'cash'
	 * and 'money'
	 *
	 * @param column the first column is 1, the second is 2...
	 *
	...
	/*
	 * Is the column a signed number? In PostgreSQL, all numbers are signed, so this
	 * is trivial. However, strings are not signed (duh!)
	 *
	 * @param column the first column is 1, the second is 2...
	 *
	 * @return true if so
	...
	private boolean populateFieldsWithMetadata(Gettable<FieldMetadata.Key, FieldMetadata> metadata) {
		...
		for (Field field : fields) {
			...
			final FieldMetadata fieldMetadata = metadata
					.get(new FieldMetadata.Key(field.getTableOid(), field.getPositionInTable()));
	...
	private void fetchFieldMetaData() throws SQLException {
		...
		StringBuilder sql = new StringBuilder("SELECT c.oid, a.attnum, a.attname, c.relname, n.nspname, "
		...
		sql.append("FROM pg_catalog.pg_class c " + "JOIN pg_catalog.pg_namespace n ON (c.relnamespace = n.oid) "
				+ "JOIN pg_catalog.pg_attribute a ON (c.oid = a.attrelid) "
				+ "JOIN pg_catalog.pg_type t ON (a.atttypid = t.oid) "
				+ "LEFT JOIN pg_catalog.pg_attrdef d ON (d.adrelid = a.attrelid AND d.adnum = a.attnum) " + "JOIN (");
		...
		try {
			...
			while (rs.next()) {
				...
				FieldMetadata fieldMetadata = new FieldMetadata(columnName, tableName, schemaName, nullable,
						autoIncrement);
	...
	/*
	 * What's a column's table's catalog name? As with getSchemaName(), we can say
	 * that if getTableName() returns n/a, then we can too - otherwise, we need to
	 * work on it.
	 *
	 * @param column the first column is 1, the second is 2...
	 *
	...
	/*
	 * Is the column definitely not writable? In reality, we would have to check the
	 * GRANT/REVOKE stuff for this to be effective, and I haven't really looked into
	 * that yet, so this will get re-visited.
	 *
	 * @param column the first column is 1, the second is 2, etc.
	 *
	 * @return true if so
	...
	/*
	 * Is it possible for a write on the column to succeed? Again, we would in
	 * reality have to check the GRANT/REVOKE stuff, which I haven't worked with as
	 * yet. However, if it isn't ReadOnly, then it is obviously writable.
	 *
	 * @param column the first column is 1, the second is 2, etc.
	 *
	 * @return true if so
	...
	/*
	 * Will a write on this column definately succeed? Hmmm...this is a bad one,
	 * since the two preceding functions have not been really defined. I cannot tell
	 * is the short answer. I thus return isWritable() just to give us an idea.
	 *
	 * @param column the first column is 1, the second is 2, etc..
	 *
	 * @return true if so
	...
	/*
	 * For several routines in this package, we need to convert a columnIndex into a
	 * Field[] descriptor. Rather than do the same code several times, here it is.
	 *
	...
	protected Field getField(int columnIndex) throws SQLException {
		if (columnIndex < 1 || columnIndex > fields.length) {
			throw new PSQLException(
					GT.tr("The column index is out of range: {0}, number of columns: {1}.", columnIndex, fields.length),
	...
	protected Integer getSQLType(int columnIndex) throws SQLException {
	 ...
	 *
	 * <code>ResultSet.getObject</code> may return a subclass of the class returned
	 * by this method.
	 *
	 * @param column
