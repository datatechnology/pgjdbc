...
public class TypeInfoCache implements TypeInfo {
	...
	// 3 - java class
	// 4 - array type oid
	private static final Object[][] types = { { "int2", Oid.INT2, Types.SMALLINT, "java.lang.Integer", Oid.INT2_ARRAY },
	...
	/**
	 * PG maps several alias to real type names. When we do queries against
	 * pg_catalog, we must use the real type, not an alias, so use this mapping.
	 */
	...
	public synchronized void addCoreType(String pgTypeName, Integer oid, Integer sqlType, String javaClass,
			Integer arrayOid) {
	...
	public synchronized int getSQLType(String pgTypeName) throws SQLException {
		...
		if (_getTypeInfoStatement == null) {
			...
			//
			String sql;
			// in case of multiple records (in different schemas) choose the one from the
			// current
			// schema,
			// otherwise take the last version of a type that is at least more deterministic
			// then before
			// (keeping old behaviour of finding types, that should not be found without
			// correct search
			// path)
			sql = "SELECT typinput='array_in'::regproc, typtype " + "  FROM pg_catalog.pg_type " + "  LEFT "
					+ "  JOIN (select ns.oid as nspoid, ns.nspname, r.r " + "          from pg_namespace as ns "
					...
					+ " WHERE typname = ? " + " ORDER BY sp.r, pg_type.oid DESC LIMIT 1;";
	...
	private PreparedStatement getOidStatement(String pgTypeName) throws SQLException {
		...
		if (dotIndex == -1 && !hasQuote && !isArray) {
			if (_getOidStatementSimple == null) {
				...
				// -- go with older way of unnesting array to be compatible with 8.0
				sql = "SELECT pg_type.oid, typname " + "  FROM pg_catalog.pg_type " + "  LEFT "
						+ "  JOIN (select ns.oid as nspoid, ns.nspname, r.r " + "          from pg_namespace as ns "
						+ "          join ( select s.r, (current_schemas(false))[s.r] as nspname "
						+ "                   from generate_series(1, array_upper(current_schemas(false), 1)) as s(r) ) as r "
						+ "         using ( nspname ) " + "       ) as sp " + "    ON sp.nspoid = typnamespace "
			...
			// default arrays are represented with _ as prefix ... this dont even work for
			// public schema
			// fully
		...
		if (isArray) {
			if (_getOidStatementComplexArray == null) {
				...
				if (_conn.haveMinimumServerVersion(ServerVersion.v8_3)) {
					sql = "SELECT t.typarray, arr.typname " + "  FROM pg_catalog.pg_type t"
					...
					sql = "SELECT t.oid, t.typname " + "  FROM pg_catalog.pg_type t"
							+ "  JOIN pg_catalog.pg_namespace n ON t.typnamespace = n.oid"
		...
		// type name requested may be schema specific, of the form
		// "{schema}"."typeName",
	...
	public synchronized String getPGType(int oid) throws SQLException {
		...
		if (_getNameStatement == null) {
			String sql;
			sql = "SELECT n.nspname = ANY(current_schemas(true)), n.nspname, t.typname " + "FROM pg_catalog.pg_type t "
		...
		if (rs.next()) {
			...
			if (onPath) {
				...
				if (schema.equals(schema.toLowerCase()) && schema.indexOf('.') == -1 && name.equals(name.toLowerCase())
						&& name.indexOf('.') == -1) {
					_pgNameToOid.put(schema + "." + name, oid);
				}
			}
	...
	/**
	 * Return the oid of the array's base element if it's an array, if not return
	 * the provided oid. This doesn't do any database lookups, so it's only useful
	 * for the originally provided type mappings. This is fine for it's intended
	 * uses where we only have intimate knowledge of types that are already known to
	 * the driver.
	 *
	 * @param oid
	 *            input oid
	...
	public int getDisplaySize(int oid, int typmod) {
		...
		case Oid.TIMESTAMPTZ:
			...
			// time = '00:00:00' = 8
			// date = '5874897-12-31' = 13 (although at large values second precision is
			// lost)
		...
		case Oid.INTERVAL:
			// SELECT LENGTH('-123456789 years 11 months 33 days 23 hours 10.123456
			// seconds'::interval);
			return 49;
		case Oid.VARCHAR:
		case Oid.BPCHAR:
	...
	/**
	 * Returns true if particular sqlType requires quoting. This method is used
	 * internally by the driver, so it might disappear without notice.
	 *
	 * @param sqlType
	 *            sql type as in java.sql.Types
