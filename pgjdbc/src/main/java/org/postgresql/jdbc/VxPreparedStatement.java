...
public class VxPreparedStatement extends VxStatement {
	...
	/**
	 * used to differentiate between new function call logic and old function call
	 * logic will be set to true if the server is &lt; 8.1 or if we are using v2
	 * protocol There is an exception to this where we are using v3, and the call
	 * does not have an out parameter before the call
	 */
	...
	VxPreparedStatement(VxConnection connection, String sql, int rsType, int rsConcurrency, int rsHoldability)
			throws SQLException {
		this(connection, connection.borrowQuery(sql), rsType, rsConcurrency, rsHoldability);
	}

	VxPreparedStatement(VxConnection connection, CachedQuery query, int rsType, int rsConcurrency, int rsHoldability)
			throws SQLException {
	...
	private void setCharacterStreamPost71(int parameterIndex, InputStream x, int length, String encoding)
			throws SQLException {
	...
	public void setObject(int parameterIndex, Object in, int targetSqlType, int scale) throws SQLException {
		...
		case Types.SQLXML:
			if (in instanceof SQLXML) {
				...
				setSQLXML(parameterIndex, new PgSQLXML(connection.createConnection(), in.toString()));
			}
			break;
		case Types.INTEGER:
		...
		case Types.TIMESTAMP_WITH_TIMEZONE:
			if (in instanceof OffsetDateTime) {
				...
				throw new PSQLException(GT.tr("Cannot cast an instance of {0} to type {1}", in.getClass().getName(),
						"Types.TIMESTAMP_WITH_TIMEZONE"), PSQLState.INVALID_PARAMETER_TYPE);
			}
			break;
		// #endif
		case Types.BOOLEAN:
		...
		case Types.BLOB:
			if (in instanceof Blob) {
				...
				throw new PSQLException(
						GT.tr("Cannot cast an instance of {0} to type {1}", in.getClass().getName(), "Types.BLOB"),
		...
		case Types.CLOB:
			if (in instanceof Clob) {
				...
				throw new PSQLException(
						GT.tr("Cannot cast an instance of {0} to type {1}", in.getClass().getName(), "Types.CLOB"),
						PSQLState.INVALID_PARAMETER_TYPE);
			}
			break;
		case Types.ARRAY:
	...
	private <A> void setPrimitiveArray(int parameterIndex, A in) throws SQLException {
		...
		if (arrayToString.supportBinaryRepresentation() && connection.getPreferQueryMode() != PreferQueryMode.SIMPLE) {
			bindBytes(parameterIndex, arrayToString.toBinaryRepresentation(connection.createConnection(), in), oid);
		} else {
	...
	private static PSQLException cannotCastException(final String fromType, final String toType,
		...
		return new PSQLException(GT.tr("Cannot convert an instance of {0} to type {1}", fromType, toType),
	 ...
	 * injection attacks. It is not done here for efficiency reasons as most calls
	 * to this method do not require escaping as the source of the string is known
	 * safe (i.e. {@code Integer.toString()})
	 *
	 * @param paramIndex
	 *            parameter index
	 * @param s
	 *            value (the value should already be escaped)
	 * @param oid
	 *            type oid
	 * @throws SQLException
	 ...
	 * directly calls bindString with no escaping; the per-protocol ParameterList
	 * does escaping as needed.
	 *
	 * @param paramIndex
	 *            parameter index
	 * @param s
	...
	public void addBatch() throws SQLException {
		...
		// we need to create copies of our parameters, otherwise the values can be
		// changed
	...
	public CompletableFuture<ResultSetMetaData> getMetaData() throws SQLException {
		...
		if (rs == null || ((VxResultSet) rs).isResultSetClosed()) {
	...
	public CompletableFuture<int[]> executeBatch() throws SQLException {
		try {
			...
			if (batchParameters != null && batchParameters.size() > 1 && m_prepareThreshold > 0) {
				// Use server-prepared statements when there's more than one statement in a
				// batch
	...
	protected void transformQueriesAndParameters() throws SQLException {
		if (batchParameters.size() <= 1 || !(preparedQuery.query instanceof BatchedQuery)) {
			return;
		}
		...
		// Typically, it does not make much sense to batch more than 128 rows:
		// performance
		// does not improve much after updating 128 statements with 1 multi-valued one,
		// thus
