...
public class PgDatabaseMetaData implements DatabaseMetaData {
			...
			try {
				...
				if (!rs.next()) {
					...
					throw new PSQLException(
							GT.tr("Unable to determine a value for MaxIndexKeys due to missing system catalog data."),
	 ...
	 * and as a result store them in mixed case? A JDBC-Compliant driver will always
	 * return false.
	 *
	 * @return true if so
	 * @throws SQLException
	 ...
	 * identifier quoting isn't supported. A JDBC Compliant driver will always use a
	...
	public String getNumericFunctions() throws SQLException {
		return EscapedFunctions.ABS + ',' + EscapedFunctions.ACOS + ',' + EscapedFunctions.ASIN + ','
				...
				+ EscapedFunctions.POWER + ',' + EscapedFunctions.RADIANS + ',' + EscapedFunctions.ROUND + ','
				+ EscapedFunctions.SIGN + ',' + EscapedFunctions.SIN + ',' + EscapedFunctions.SQRT + ','
				+ EscapedFunctions.TAN + ',' + EscapedFunctions.TRUNCATE;

	}

	public String getStringFunctions() throws SQLException {
		String funcs = EscapedFunctions.ASCII + ',' + EscapedFunctions.CHAR + ',' + EscapedFunctions.CONCAT + ','
				+ EscapedFunctions.LCASE + ',' + EscapedFunctions.LEFT + ',' + EscapedFunctions.LENGTH + ','
				+ EscapedFunctions.LTRIM + ',' + EscapedFunctions.REPEAT + ',' + EscapedFunctions.RTRIM + ','
				+ EscapedFunctions.SPACE + ',' + EscapedFunctions.SUBSTRING + ',' + EscapedFunctions.UCASE;
	...
	public String getTimeDateFunctions() throws SQLException {
		String timeDateFuncs = EscapedFunctions.CURDATE + ',' + EscapedFunctions.CURTIME + ','
				+ EscapedFunctions.DAYNAME + ',' + EscapedFunctions.DAYOFMONTH + ',' + EscapedFunctions.DAYOFWEEK + ','
				+ EscapedFunctions.DAYOFYEAR + ',' + EscapedFunctions.HOUR + ',' + EscapedFunctions.MINUTE + ','
				+ EscapedFunctions.MONTH + ',' + EscapedFunctions.MONTHNAME + ',' + EscapedFunctions.NOW + ','
				+ EscapedFunctions.QUARTER + ',' + EscapedFunctions.SECOND + ',' + EscapedFunctions.WEEK + ','
	 ...
	 * identifier, so we can't possibly list them all.
	 *
	 * From the file src/backend/parser/scan.l, an identifier is ident_start
	 * [A-Za-z\200-\377_] ident_cont [A-Za-z\200-\377_0-9\$] identifier
	 ...
	 * <p>
	 * <a href=
	 * "http://www.microsoft.com/msdn/sdk/platforms/doc/odbc/src/intropr.htm">http://www.
	 ...
	 * Compliant drivers must return true. We currently report false until 'schema'
	 * support is added. Then this should be changed to return true, since we will
	 * be mostly compliant (probably more compliant than many other databases) And
	 * since this is a requirement for all JDBC drivers we need to get to the point
	 * where we can return true.
	 *
	 * @return true if connected to PostgreSQL 7.3+
	 * @throws SQLException
	 *             if a database access error occurs
	 ...
	 * this means support for constraints
	 *
	 * @return true
	 *
	 ...
	 * guarantee that. In further reflection. we are talking a Statement object
	 ...
	 * guarantee that. In further contemplation, we are talking a Statement object
	 * here, so the answer is yes, since the Statement is only a vehicle to
	 * ExecSQL() in Connection
	 *
	 * @return true
	 */
	 ...
	 * more than 1600 attributes (realistically, this is limited by the fact that
	 * tuple sizes must be less than 8192 bytes)..."
	 *
	 ...
	 * database? Well, since it depends on postmaster, which just does a listen()
	 * followed by an accept() and fork(), its basically very high. Unless the
	 * system runs out of processes, it can be 65535 (the number of aux. ports on a
	 * TCP/IP system). I will return 8192 since that is what even the largest system
	 ...
	 * before 8.0; from 8.0 READ_UNCOMMITTED and REPEATABLE_READ are accepted
	 * aliases for READ_COMMITTED.
	 */
	 ...
	 * to commit? It seems to mean something like:
	 *
	 * <pre>
	 ...
	 * a query. This includes the single quotes needed around it.
	 *
	 * @param s
	 *            input value
	 *
	...
	public ResultSet getProcedures(String catalog, String schemaPattern, String procedureNamePattern)
		...
		sql = "SELECT NULL AS PROCEDURE_CAT, n.nspname AS PROCEDURE_SCHEM, p.proname AS PROCEDURE_NAME, "
				+ "NULL, NULL, NULL, d.description AS REMARKS, " + DatabaseMetaData.procedureReturnsResult
				+ " AS PROCEDURE_TYPE, " + " p.proname || '_' || p.oid AS SPECIFIC_NAME "
	...
	public ResultSet getTables(String catalog, String schemaPattern, String tableNamePattern, String[] types)
		...
		select = "SELECT NULL AS TABLE_CAT, n.nspname AS TABLE_SCHEM, c.relname AS TABLE_NAME, "
				...
				+ "  WHEN 'r' THEN 'SYSTEM TABLE' " + "  WHEN 'v' THEN 'SYSTEM VIEW' "
				...
				+ "  WHEN 'r' THEN 'TEMPORARY TABLE' " + "  WHEN 'p' THEN 'TEMPORARY TABLE' "
				+ "  WHEN 'i' THEN 'TEMPORARY INDEX' " + "  WHEN 'S' THEN 'TEMPORARY SEQUENCE' "
				+ "  WHEN 'v' THEN 'TEMPORARY VIEW' " + "  ELSE NULL " + "  END " + " END "
				+ " WHEN false THEN CASE c.relkind " + " WHEN 'r' THEN 'TABLE' " + " WHEN 'p' THEN 'TABLE' "
				+ " WHEN 'i' THEN 'INDEX' " + " WHEN 'S' THEN 'SEQUENCE' " + " WHEN 'v' THEN 'VIEW' "
				+ " WHEN 'c' THEN 'TYPE' " + " WHEN 'f' THEN 'FOREIGN TABLE' " + " WHEN 'm' THEN 'MATERIALIZED VIEW' "
				+ " ELSE NULL " + " END " + " ELSE NULL " + " END " + " AS TABLE_TYPE, d.description AS REMARKS "
	...
	/**
	 * PostgreSQL does not support multiple catalogs from a single connection, so to
	 * reduce confusion we only return the current catalog. {@inheritDoc}
	 */
	@Override
	...
	public ResultSet getColumns(String catalog, String schemaPattern, String tableNamePattern, String columnNamePattern)
		...
		while (rs.next()) {
			...
			tuple[10] = connection
					.encodeString(Integer.toString(rs.getBoolean("attnotnull") ? java.sql.DatabaseMetaData.columnNoNulls
		...
		createMetaDataStatement()).createDriverResultSet(f, v);
	}
	...
	public ResultSet getColumnPrivileges(String catalog, String schema, String table, String columnNamePattern)
		...
		sql = "SELECT n.nspname,c.relname,r.rolname,c.relacl, "
				+ (connection.haveMinimumServerVersion(ServerVersion.v8_4) ? "a.attacl, " : "") + " a.attname "
				+ " FROM pg_catalog.pg_namespace n, pg_catalog.pg_class c, "
				+ " pg_catalog.pg_roles r, pg_catalog.pg_attribute a " + " WHERE c.relnamespace = n.oid "
				+ " AND c.relowner = r.oid " + " AND c.oid = a.attrelid " + " AND c.relkind = 'r' "
	...
	public ResultSet getTablePrivileges(String catalog, String schemaPattern, String tableNamePattern)
		...
		sql = "SELECT n.nspname,c.relname,r.rolname,c.relacl "
				...
				+ " WHERE c.relnamespace = n.oid " + " AND c.relowner = r.oid " + " AND c.relkind IN ('r','p') ";
	 ...
	 * SQL permission name to a List of usernames who have that permission. For
	 * instance: {@code SELECT -> user1 -> list of [grantor, grantable]}
	 *
	 * @param aclArray
	 *            ACL array
	 * @param owner
	...
	public Map<String, Map<String, List<String[]>>> parseACL(String aclArray, String owner) {
		...
		Map<String, Map<String, List<String[]>>> privileges = new HashMap<String, Map<String, List<String[]>>>();
		for (String acl : acls) {
	...
	public ResultSet getBestRowIdentifier(String catalog, String schema, String table, int scope, boolean nullable)
		 ...
		 * believe other unique indexes, ctid, and oid should also be considered. -KJ
		 */

		String sql;
		sql = "SELECT a.attname, a.atttypid, atttypmod " + "FROM pg_catalog.pg_class ct "
				+ "  JOIN pg_catalog.pg_attribute a ON (ct.oid = a.attrelid) "
				+ "  JOIN pg_catalog.pg_namespace n ON (ct.relnamespace = n.oid) "
				+ "  JOIN (SELECT i.indexrelid, i.indrelid, i.indisprimary, "
				+ "             information_schema._pg_expandarray(i.indkey) AS keys "
				+ "        FROM pg_catalog.pg_index i) i "
				+ "    ON (a.attnum = (i.keys).x AND a.attrelid = i.indrelid) " + "WHERE true ";

		if (schema != null && !schema.isEmpty()) {
			sql += " AND n.nspname = " + escapeQuotes(schema);
		}

		sql += " AND ct.relname = " + escapeQuotes(table) + " AND i.indisprimary " + " ORDER BY a.attnum ";
	...
	public ResultSet getVersionColumns(String catalog, String schema, String table) throws SQLException {
		 ...
		 * some databases' timestamp type. We can't tell what rules or triggers might be
		 * doing, so we are left with the system columns that change on an update. An
		 * update may change all of the following system columns: ctid, xmax, xmin,
		 * cmax, and cmin. Depending on if we are in a transaction and whether we roll
		 * it back or not the only guaranteed change is to ctid. -KJ
		 */
	 ...
	 *             if something wrong happens
	 */
	protected ResultSet getImportedExportedKeys(String primaryCatalog, String primarySchema, String primaryTable,
			String foreignCatalog, String foreignSchema, String foreignTable) throws SQLException {

		/*
		 * The addition of the pg_constraint in 7.3 table should have really helped us
		 * out here, but it comes up just a bit short. - The conkey, confkey columns
		 * aren't really useful without contrib/array unless we want to issues separate
		 * queries. - Unique indexes that can support foreign keys are not necessarily
		 * added to pg_constraint. Also multiple unique indexes covering the same keys
		 * can be created which make it difficult to determine the PK_NAME field.
		 */
		...
		String sql = "SELECT NULL::text AS PKTABLE_CAT, pkn.nspname AS PKTABLE_SCHEM, pkc.relname AS PKTABLE_NAME, pka.attname AS PKCOLUMN_NAME, "
				...
				+ "pos.n AS KEY_SEQ, " + "CASE con.confupdtype " + " WHEN 'c' THEN "
				+ DatabaseMetaData.importedKeyCascade + " WHEN 'n' THEN " + DatabaseMetaData.importedKeySetNull
				+ " WHEN 'd' THEN " + DatabaseMetaData.importedKeySetDefault + " WHEN 'r' THEN "
				+ DatabaseMetaData.importedKeyRestrict + " WHEN 'p' THEN " + DatabaseMetaData.importedKeyRestrict
				+ " WHEN 'a' THEN " + DatabaseMetaData.importedKeyNoAction + " ELSE NULL END AS UPDATE_RULE, "
				+ "CASE con.confdeltype " + " WHEN 'c' THEN " + DatabaseMetaData.importedKeyCascade + " WHEN 'n' THEN "
				+ DatabaseMetaData.importedKeySetNull + " WHEN 'd' THEN " + DatabaseMetaData.importedKeySetDefault
				+ " WHEN 'r' THEN " + DatabaseMetaData.importedKeyRestrict + " WHEN 'p' THEN "
				+ DatabaseMetaData.importedKeyRestrict + " WHEN 'a' THEN " + DatabaseMetaData.importedKeyNoAction
				+ " ELSE NULL END AS DELETE_RULE, " + "con.conname AS FK_NAME, pkic.relname AS PK_NAME, " + "CASE "
				+ " WHEN con.condeferrable AND con.condeferred THEN " + DatabaseMetaData.importedKeyInitiallyDeferred
				+ " WHEN con.condeferrable THEN " + DatabaseMetaData.importedKeyInitiallyImmediate + " ELSE "
				+ DatabaseMetaData.importedKeyNotDeferrable + " END AS DEFERRABILITY " + " FROM "
				+ " pg_catalog.pg_namespace pkn, pg_catalog.pg_class pkc, pg_catalog.pg_attribute pka, "
				+ " pg_catalog.pg_namespace fkn, pg_catalog.pg_class fkc, pg_catalog.pg_attribute fka, "
				+ " pg_catalog.pg_constraint con, " + " pg_catalog.generate_series(1, " + getMaxIndexKeys()
				+ ") pos(n), " + " pg_catalog.pg_class pkic";
	...
	public ResultSet getCrossReference(String primaryCatalog, String primarySchema, String primaryTable,
			String foreignCatalog, String foreignSchema, String foreignTable) throws SQLException {
		return getImportedExportedKeys(primaryCatalog, primarySchema, primaryTable, foreignCatalog, foreignSchema,
				foreignTable);
	}
		...
		if (connection.haveMinimumServerVersion(ServerVersion.v8_3)) {
			sql = "SELECT NULL AS TABLE_CAT, n.nspname AS TABLE_SCHEM, "
					...
					+ (connection.haveMinimumServerVersion(ServerVersion.v9_6)
							? "  CASE am.amname " + "    WHEN 'btree' THEN CASE i.indoption[(i.keys).n - 1] & 1 "
									+ "      WHEN 1 THEN 'D' " + "      ELSE 'A' " + "    END " + "    ELSE NULL "
									+ "  END AS ASC_OR_DESC, "
							: "  CASE am.amcanorder " + "    WHEN true THEN CASE i.indoption[(i.keys).n - 1] & 1 "
									+ "      WHEN 1 THEN 'D' " + "      ELSE 'A' " + "    END " + "    ELSE NULL "
									+ "  END AS ASC_OR_DESC, ")
	...
	public ResultSet getUDTs(String catalog, String schemaPattern, String typeNamePattern, int[] types)
		...
		String sql = "select "
				...
				+ "CASE WHEN t.typtype='c' then " + java.sql.Types.STRUCT + " else " + java.sql.Types.DISTINCT
	...
	public ResultSet getFunctions(String catalog, String schemaPattern, String functionNamePattern)
		...
		if (pgFuncResultExists) {
			funcTypeSql = " CASE " + "   WHEN (format_type(p.prorettype, null) = 'unknown') THEN "
					+ DatabaseMetaData.functionResultUnknown + "   WHEN "
					...
					+ DatabaseMetaData.functionReturnsTable + "   ELSE " + DatabaseMetaData.functionNoTable + " END ";
		}

		// Build query and result
		String sql;
		sql = "SELECT current_database() AS FUNCTION_CAT, n.nspname AS FUNCTION_SCHEM, p.proname AS FUNCTION_NAME, "
				+ " d.description AS REMARKS, " + funcTypeSql + " AS FUNCTION_TYPE, "
				+ " p.proname || '_' || p.oid AS SPECIFIC_NAME " + "FROM pg_catalog.pg_proc p "
				+ "INNER JOIN pg_catalog.pg_namespace n ON p.pronamespace=n.oid "
				+ "LEFT JOIN pg_catalog.pg_description d ON p.oid=d.objoid " + "WHERE pg_function_is_visible(p.oid) ";
		if (schemaPattern != null && !schemaPattern.isEmpty()) {
			sql += " AND n.nspname LIKE " + escapeQuotes(schemaPattern);
		}
	...
	public boolean locatorsUpdateCopy() throws SQLException {
		 ...
		 * CachedRowSet calls this method regardless of whether large objects are used.
		 */
		return true;
	}
