...
class VxCallableStatement extends VxPreparedStatement {
	...
	VxCallableStatement(VxConnection vxConnection, String sql, int rsType, int rsConcurrency, int rsHoldability)
			throws SQLException {
		super(vxConnection, vxConnection.borrowCallableQuery(sql), rsType, rsConcurrency, rsHoldability);
	...
	public CompletableFuture<Boolean> executeWithFlags(int flags) throws SQLException {
		...
		if (cols != outParameterCount) {
			throw new PSQLException(GT.tr("A CallableStatement was executed with an invalid number of parameters"),
	 ...
	 * method whose Java type XXX corresponds to the parameter's registered SQL
	 * type.
	 *
	 ...
	 * @param sqlType
	 *            SQL type code defined by java.sql.Types; for parameters of type
	 *            Numeric or Decimal use the version of registerOutParameter that
	...
	public void registerOutParameter(int parameterIndex, int sqlType, boolean setPreparedParameters)
		...
		if (!isFunction) {
			throw new PSQLException(
					GT.tr("This statement does not declare an OUT parameter.  Use '{' ?= call ... '}' to declare one."),
	 ...
	 * @param setPreparedParameters
	 *            set prepared parameters
	 * @throws SQLException
	 *             if a database-access error occurs.
	 */
	public void registerOutParameter(int parameterIndex, int sqlType, int scale, boolean setPreparedParameters)
			throws SQLException {
		...
		if (type1 != this.testReturn[parameterIndex - 1] && type2 != this.testReturn[parameterIndex - 1]) {
			throw new PSQLException(
					GT.tr("Parameter of type {0} was registered, but call to get{1} (sqltype={2}) was made.",
							"java.sql.Types=" + testReturn[parameterIndex - 1], getName, "java.sql.Types=" + type1),
	...
	protected void checkIndex(int parameterIndex, int type, String getName) throws SQLException {
		...
		if (type != this.testReturn[parameterIndex - 1]) {
			throw new PSQLException(
					GT.tr("Parameter of type {0} was registered, but call to get{1} (sqltype={2}) was made.",
							"java.sql.Types=" + testReturn[parameterIndex - 1], getName, "java.sql.Types=" + type),
					PSQLState.MOST_SPECIFIC_TYPE_DOES_NOT_MATCH);
		}
	}
	 ...
	 *
	 * @param parameterIndex
	 *            index of getXXX (index) check to make sure is a function and index
	 *            == 1
	 * @param fetchingData
	 *            fetching data
	 */
	...
	protected VxBatchResultHandler createBatchHandler(Query[] queries, ParameterList[] parameterLists) {
		return new VxCallableBatchResultHandler(this, queries, parameterLists);
	}
