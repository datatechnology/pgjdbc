...
class GssAction implements PrivilegedAction<Exception> {
	...
	GssAction(PGStream pgStream, GSSCredential clientCredentials, String host, String user, String kerberosServerName,
			boolean useSpnego) {
		...
		try {
			...
			while (!established) {
				...
				if (!secContext.isEstablished()) {
					...
					try {
						int response = pgStream.receiveChar().get();

						// Error
						switch (response) {
						case 'E':
							int l_elen = pgStream.receiveInteger4().get();
							ServerErrorMessage l_errorMsg = new ServerErrorMessage(
									pgStream.receiveErrorString(l_elen - 4).get());
						...
						case 'R':
							...
							int len = pgStream.receiveInteger4().get();
							int type = pgStream.receiveInteger4().get();
							// should check type = 8
							inToken = pgStream.receive(len - 8).get();
							break;
						default:
