...
public class VxAutoRollbackTestSuite extends VxBaseTest4 {
  ...
  public VxAutoRollbackTestSuite(AutoSave autoSave, AutoCommit autoCommit,
  ...
  public void setUp() throws Exception {
    ...
    VxTestUtil.createTable(con, "rollbacktest", "a int, str text");
    con.setAutoCommit(autoCommit == AutoCommit.YES).get();
    VxConnection baseConnection = con.unwrap(VxConnection.class);
    baseConnection.setFlushCacheOnDeallocate(flushCacheOnDeallocate);
    Assume.assumeTrue("DEALLOCATE ALL requires PostgreSQL 8.3+",
        failMode != FailMode.DEALLOCATE || VxTestUtil.haveMinimumServerVersion(con, ServerVersion.v8_3));
    Assume.assumeTrue("DISCARD ALL requires PostgreSQL 8.3+",
        failMode != FailMode.DISCARD || VxTestUtil.haveMinimumServerVersion(con, ServerVersion.v8_3));
    Assume.assumeTrue("Plan invalidation on table redefinition requires PostgreSQL 8.3+",
        failMode != FailMode.ALTER || VxTestUtil.haveMinimumServerVersion(con, ServerVersion.v8_3));
  }
  ...
  public void tearDown() throws SQLException {
    try {
      ...
      VxTestUtil.dropTable(con, "rollbacktest");
    } catch (Exception e) {
      e.printStackTrace();
    }
    super.tearDown();
  ...
  public static Iterable<Object[]> data() {
    ...
    */
    ...
    AutoCommit autoCommit = AutoCommit.YES;
    FailMode failMode = FailMode.ALTER;
    ContinueMode continueMode = ContinueMode.SELECT;
    boolean flushCacheOnDeallocate = true;
    boolean trans = false;
  ...
  @Test
  public void run() throws SQLException, InterruptedException, ExecutionException {
    ...
    statement.executeUpdate("insert into rollbacktest(a, str) values (0, 'test')").get();
    int rowsExpected = 1;

    VxPreparedStatement ps = con.prepareStatement(testSql.getSql(cols));
    // Server-prepare the testSql
    ps.executeQuery().get().close();
    rowsExpected += testSql.rowsInserted;

    if (trans) {
      statement.executeUpdate("update rollbacktest set a=a").get();
    }

    switch (failMode) {
      case SELECT:
        try {
          statement.execute("select 1/0").get();
        ...
        break;
      case DEALLOCATE:
        statement.executeUpdate("DEALLOCATE ALL").get();
        break;
      case DISCARD:
        statement.executeUpdate("DISCARD ALL").get();
        break;
      case ALTER:
        statement.executeUpdate("alter table rollbacktest add q int").get();
        break;
      case INSERT_BATCH:
        try {
          statement.addBatch("insert into rollbacktest(a, str) values (1/0, 'test')");
          statement.executeBatch().get();
          Assert.fail("select 1/0 should fail");
        } catch (SQLException e) {
          Assert.assertEquals("division by zero expected",
              PSQLState.DIVISION_BY_ZERO.getState(), e.getSQLState());
    ...
    VxConnection pgConnection = con.unwrap(VxConnection.class);
    ...
    switch (continueMode) {
      ...
      case IS_VALID:
        if (!flushCacheOnDeallocate && autoSave == AutoSave.NEVER
            ...
            && con.unwrap(VxConnection.class).getPreferQueryMode() != PreferQueryMode.SIMPLE) {
  ...
  private void assertRows(String tableName, int nrows) throws SQLException, InterruptedException, ExecutionException {
    VxStatement st = con.createStatement();
    VxResultSet rs = st.executeQuery("select count(*) from " + tableName).get();
    rs.next().get();
    Assert.assertEquals("Table " + tableName, nrows, (Object)rs.getInt(1).get());
  }

  private void doCommit() throws SQLException, InterruptedException, ExecutionException {
    ...
    if (con.getAutoCommit()) {
      ...
      VxStatement st = con.createStatement();
      st.executeUpdate(
          "insert into rollbacktest(a, str) values (42, '" + System.currentTimeMillis() + "," + counter.getAndIncrement() + "')").get();
      st.close();
    }
    con.commit().get();
    con.setAutoCommit(autoCommit == AutoCommit.YES).get();
